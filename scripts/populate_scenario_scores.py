...
from paths import ASVSPOOF_LA, DEV_FLAC  # noqa: E402
...
def _resolve_audio(value: str) -> Path:
    """清单保存数据根目录下的相对路径，避免提交个人机器绝对路径。"""
    path = Path(value)
    return path if path.is_absolute() else ASVSPOOF_LA / path
...
def main():
    ...
    for row in rows:
        ...
        if value and value != "TODO":
            row["audio"] = str(_resolve_audio(value))
            rows.append(row)
    if not rows:
