...
from paths import (SENSEVOICE_DIR, OLLAMA_HOST, OLLAMA_MODEL, DEVICE,  # noqa: E402
                   SEMANTIC_LLM_BACKEND, is_offline,
...
class SemanticChannel:
    ...
    def __init__(self, model: Optional[str] = None, url: Optional[str] = None,
        ...
        self.cloud_base = ""
        self.cloud_key = ""
        self._refresh_backend_config()
        self.timeout = timeout
        self.asr = asr or SenseVoiceASR()
        self._llm_cache: Optional[bool] = None
        self._llm_cache_key: tuple | None = None

    def _refresh_backend_config(self) -> None:
        """在调用时刷新 secrets/env 配置，避免模块导入时冻结密钥。"""
        base, key, cloud_model = semantic_cloud_config()
        self.cloud_base = base
        self.cloud_key = key
        if self.backend == "cloud":
            self.model = self._model_override or cloud_model
            self.url = self._url_override or f"{base}/chat/completions"
        else:
            self.model = self._model_override or DEFAULT_MODEL
            self.url = self._url_override or OLLAMA_URL
    ...
    def analyze(self, transcript: str) -> SemanticResult:
        ...
        self._refresh_backend_config()
        if is_offline() or not self.llm_available():
    ...
    def llm_available(self) -> bool:
        ...
        if is_offline():
            return False
        cache_key = (self.backend, self.url, self.cloud_key)
        if self._llm_cache is not None and self._llm_cache_key == cache_key:
            return self._llm_cache
        ok = False
        if self.backend == "cloud":
            ok = bool(self.cloud_key)
        else:
            try:
