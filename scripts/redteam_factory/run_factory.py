...
def _prompt_for(ref_audio: Path) -> str:
    ...
    if refs_json.exists():
        ...
        try:
            refs = _json.loads(refs_json.read_text(encoding="utf-8"))
            try:
                key = ref_audio.resolve().relative_to(ROOT.resolve()).as_posix()
            except ValueError:
                key = str(ref_audio).replace("\\", "/")
            entry = refs.get(key) or refs.get(str(ref_audio).replace("\\", "/")) or {}
