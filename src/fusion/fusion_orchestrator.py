...
class FusionOrchestrator:
    ...
    def decide(self, acoustic: ChannelVerdict,
        ...
        number: NumberChannel.check() 的返回（None 或含
        action/matched/to_channel_dict）。只有 ``action == "block"`` 的精确黑名单
        命中才短路；启发式命中为 ``caution``，继续跑三通道并把提示保留到最终结果。
        未命中/无号码 → 原逻辑三通道融合。
        """
        number_action = str(getattr(number, "action", "none") or "none")
        if number is not None and number_action == "block":
            return FusionResult(
                final="block",
        ...
        for c in chans:
            if c.score >= self.hard_block and c.confidence >= 0.6:
                result_channels = [c.to_dict() for c in chans]
                rationale = (
                    f"纵深防御拦截：{c.name} 通道高置信命中"
                    f"（{c.label}，{c.detail}）"
                )
                if number is not None and number_action == "caution":
                    result_channels.insert(0, number.to_channel_dict())
                    rationale = (
                        f"⓪启发式号码提示（{number.source}：{number.detail}）；"
                        f"{rationale}"
                    )
                return FusionResult(
                    final="block",
        ...
        result_channels = [c.to_dict() for c in chans]
        if number is not None and number_action == "caution":
            # 启发式只提供背景风险，不允许单独硬拦；但最终结论至少为 caution。
            if final == "allow":
                final = "caution"
            number_dict = number.to_channel_dict()
            result_channels.insert(0, number_dict)
            rationale = (
                f"⓪启发式号码提示（{number.source}：{number.detail}）；"
                f"{rationale}"
            )
