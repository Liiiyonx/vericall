...
| 数据源 | 真/伪 | 本地状态 | 可用性 |
|---|---|---|---|
| aishell1（真） | 纯真 | ✅ **AISHELL-1 全量 100 说话人 34,715 条**（S0002-S0101，hf-mirror 2026-09-06） | **合规中文真样本池（可入训练，CFAD 真同源）** |
| FMFCC-A（伪） | 4 万伪(A 系) + 1 万真(N01) | ✅ **17,636 伪 wav** | **合规中文伪样本池（可入训练）** |
| CFAD | 1000 真 + 1000 伪 | 完整（2000 条 + 4 退化布局） | **评测集**，按隔离纪律不作训练 |
| 红队 | 全伪 | 5238 独立样本（母本 3117 + 切段 2121） | **禁入训练**，作评测难例集 |
| ASVspoof LA | 英文真伪 | 完整（train/dev/eval） | 英文基线已用 |

**结论（2026-09-06 终版）**：中文域增量训练数据侧**已完全闭环**——真 aishell1 34,715（全量）+ 伪 FMFCC 17.6k。
已跑通并产出：同域说话人外 EER 0.00%、红队击穿 0.0%（基线 95.5-99%）、留一系统检出 100%，
正式打分器 `cn_lr_scorer_full.pkl`（见 `evaluation/redblue_evolution.md`）。
