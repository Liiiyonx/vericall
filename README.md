...
流式实时拦截（VAD/WS + 增量融合状态机）已实现并通过端到端测试：3s 滑窗 / 1s 步进，P90 延迟 ≤2s，系统支持"边说边判"。
...
├── evaluation/            # 指标(metrics) + 跨攻击域评测 + 红队评测运行器（纯代码）
├── scripts/               # 下载 / 预处理 / 训练 / 评估 / 监控 / 红队工厂
├── tests/                 # pytest 单测（融合决策 + 评测指标 + 流式状态机）
├── web/                   # 单页演示前端（老人端 / 子女端 / 竞技场）
└── data/demo_cache.json   # 离线演示缓存（A/B/C 三场景）
```
...
ASVspoof 许可**禁止再分发**，demo 不打包数据集里的 flac。运行 `scripts/prepare_demo_audio.py` 可把本机 LA 数据拷贝到 `assets/demo_audio/`（仅本地用，`.gitignore` 排除）；没有数据集时可用 GPT-SoVITS 自生成演示音频。
...
# 电话信道退化（phone8k/noise 纯 numpy；mp3_16k/amr 需 ffmpeg）
