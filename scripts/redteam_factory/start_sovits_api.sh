...
set -a; source "${VERICALL_SECRETS_FILE:-$HOME/VeriCall_data/secrets/vericall_secrets.env}" 2>/dev/null; set +a

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="${VERICALL_SOVITS_ROOT:-$SCRIPT_DIR/../../external/GPT-SoVITS}"
PY="${VERICALL_SOVITS_PYTHON:-python}"
TORCH_SITE="${VERICALL_TORCH_SITE:-$HOME/VeriCall_data/pyenv/torch_site2}"
export PYTHONPATH="$TORCH_SITE"
export NLTK_DATA="${VERICALL_NLTK_DATA:-$TORCH_SITE/nltk_data}"
