...
from paths import SV_EXAMPLE, DEVICE, is_offline, DEMO_CACHE  # noqa: E402
...
class VeriCallPipeline:
    ...
    # ------------------------------------------------------------------ #
    def _voiceprint_verdict(
            self, audio_path: str,
            allowed_names: Iterable[str] | None = None) -> ChannelVerdict:
        """通道② 真实核验（CAMPPlus ~0.3GB，常驻不卸载）。"""
        if allowed_names is None:
            return self.vp.verify(audio_path)
        return self.vp.verify(audio_path, allowed_names=allowed_names)
    ...
    def _semantic_verdict(self, audio_path: str) -> ChannelVerdict:
        ...
        try:
            ...
            reason = ""
        # 云端 LLM 不占本机显存，ASR 应常驻；只有本地 Ollama 的逃生口
        # 才需要在大模型冷加载前腾显存。
        should_unload_asr = _FORCE_UNLOAD_ASR or (
            self.sem.backend == "ollama"
            and _free_vram_bytes() < _LLM_VRAM_BYTES
        )
        if self.sem.asr._model is not None and should_unload_asr:
    ...
    def _number_verdict(self, caller_number: str | None,
        ...
        v = self._number_ch.check(str(caller_number), family_id=family_id)
        print(f"[通道⓪ 号码] {caller_number} -> {v.source} (score={v.score} "
              f"conf={v.confidence}, action={v.action}) {v.detail}")
        return v

    # ------------------------------------------------------------------ #
    def _three_channels_sequential(
            self, audio_path: str,
        ...
        voiceprint = self._voiceprint_verdict(audio_path, allowed_names)
    ...
    def _three_channels_parallel(
        ...
        def _vp():
            v = self._voiceprint_verdict(audio_path, allowed_names)
    ...
    def analyze(self, audio_path: str, scenario: str | None = None,
                ...
                allowed_names: Iterable[str] | None = None,
                family_id: str | None = None) -> "FusionResult":
        ...
        """
        # 通道⓪ 号码先验（本地查表 0ms；仅精确黑名单命中时短路）
        number = self._number_verdict(caller_number, family_id=family_id)
        if number is not None and number.action == "block":
            return self.orch.decide(
                ...
                number=number)
        if is_offline():
            return self._offline_result(scenario, number=number)
        print(f"\n=== 分析音频: {audio_path} ===")

        parallel = os.environ.get("VERICALL_PARALLEL", "0") == "1"
        if parallel:
            acoustic, voiceprint, semantic = self._three_channels_parallel(
                audio_path, allowed_names=allowed_names)
        else:
            acoustic, voiceprint, semantic = self._three_channels_sequential(
                audio_path, allowed_names=allowed_names)

        # 融合裁决
        result = self.orch.decide(acoustic, voiceprint, semantic, number=number)
    ...
    # ------------------------------------------------------------------ #
    def _offline_result(self, scenario: str | None, number=None) -> "FusionResult":
