...
class _StubAc:
    ...
    def analyze(self, path, unload_after=True):
        ...
        with wave.open(path, "rb") as wf:
            self.sample_rates.append(wf.getframerate())
...
def test_tail_window_written_with_true_sample_rate():
    ...
    try:
        ...
        assert pipe.ac.sample_rates[-1] == SR, \
            f"尾窗采样率错误: {pipe.ac.sample_rates[-1]}（修复前为 24000//2=12000 类错误值）"
        assert Path(pipe.ac.calls[-1][0]).name == "acoustic.wav"
        assert Path(pipe.ac.calls[-1][0]).exists()
    finally:
        backends["_cleanup"]()
    assert not Path(pipe.ac.calls[-1][0]).exists()


def test_all_stream_channels_reuse_and_cleanup_temp_wav():
    pipe = _StubPipe()
    backends = make_real_backends(pipe, offline=True, sr=SR)
    w = _win(0, 3.0)
    try:
        backends["acoustic"](w)
        backends["voiceprint"](w)
        backends["semantic"](w, "")

        paths = [pipe.ac.calls[-1][0], pipe.vp.calls[-1], pipe.sem.asr.calls[-1]]
        assert len(set(paths)) == 3
        assert {Path(path).name for path in paths} == {
            "acoustic.wav", "voiceprint.wav", "semantic.wav"}
...
def test_stale_stream_wavs_cleaned_on_backend_init():
    """常开守护启动时兜底清理遗留的旧窗口文件。"""
    tmp = Path(tempfile.gettempdir()) / "vericall_stream"
    tmp.mkdir(parents=True, exist_ok=True)
    stale = tmp / "win_stale_regression.wav"
    stale.write_bytes(b"stale")
    old = time.time() - 7200
    stale.touch()
    stale.chmod(stale.stat().st_mode)
    stale_dir = tmp / "stream_stale_regression"
    stale_dir.mkdir(exist_ok=True)
    import os
    os.utime(stale, (old, old))
    os.utime(stale_dir, (old, old))

    backends = make_real_backends(_StubPipe(), offline=False, sr=SR)
    backends["_cleanup"]()
