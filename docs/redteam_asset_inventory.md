...
| 数据源 | 真/伪 | 本地状态 | 可用性 |
|---|---|---|---|
| FMFCC-A | 40k 真 + 10k 伪（70 攻击型） | 仅 17,636 **真** wav | 伪样本未下载（整包 zip 数 GB+，见 `docs/数据集获取指南.md`） |
| CFAD | 1000 真 + 1000 伪 | 完整（2000 条 + 4 退化布局） | **评测集**，按隔离纪律不作训练 |
| 红队 | 全伪 | 4605 wav | **禁入训练**，作评测难例集 |
| ASVspoof LA | 英文真伪 | 完整（train/dev/eval） | 英文基线已用 |

**结论**：中文域增量训练（把 CFAD EER 15.5% 拉向 <5%）的数据侧前置 = **补齐 FMFCC-A 伪样本 10k**（或其子集含 N01 等神经合成），
否则「红蓝循环第 1 轮」训练侧无法开工（真样本已足够：FMFCC 真 17.6k 可作真侧，缺中文伪侧）。
