...
from private_fs import ensure_private_dir, write_private_text  # noqa: E402
...
class VoiceprintChannel:
    ...
    def _save_profile(self, name: str, vec: np.ndarray, source_file: str = "") -> None:
        ...
        try:
            os.chmod(self._profile_vec_path(name), 0o600)
        except OSError:
            pass
    ...
    def _eligible_names(
            self, allowed_names: Iterable[str] | None) -> Optional[set[str]]:
    ...
    def verify(self, audio_path: str,
        ...
        eligible = self._eligible_names(allowed_names)
        if eligible is not None and not any(
                name in eligible for name in self.profiles):
            return ChannelVerdict(
                name="voiceprint", score=0.0, label="stub",
                detail="当前家庭未登记任何家人声纹", confidence=0.0,
            )
        try:
            vec = self._embed(audio_path)
        ...
        best_name, best_sim = self._best_match(vec, eligible)
    ...
    def _best_match(
            self, vec: np.ndarray,
            allowed_names: set[str] | None = None) -> Tuple[str, float]:
    ...
    def similarity(
        ...
        """返回 (最似家人名, 余弦相似度, 错误信息)。供挑战应答等需要原始相似度的上层使用。

        与 verify() 的区别：不回传映射后的可疑度分数，直接给原始余弦，
        challenge-response 用它与 VOICE_THR 比较判定"是否本人实时跟读"。
        """
        if not self.profiles:
            return None, 0.0, "未登记任何家人声纹"
        eligible = self._eligible_names(allowed_names)
        if eligible is not None and not any(
                name in eligible for name in self.profiles):
            return None, 0.0, "当前家庭未登记任何家人声纹"
        try:
            vec = self._embed(audio_path)
        except Exception as e:  # noqa: BLE001
            return None, 0.0, f"声纹提取失败: {e}"
        best_name, best_sim = self._best_match(vec, eligible)
        return best_name, float(best_sim), None
