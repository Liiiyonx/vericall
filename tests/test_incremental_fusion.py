...
def test_offline_flag_is_per_window_and_resets():
    f = IncrementalFusion()
    ac = ChannelVerdict("acoustic", 0.05, "normal", "stub", 0.0)
    vp = _cv("voiceprint", 0.05)
    sem = _cv("semantic", 0.05)
    evs = f.update(ac, vp, sem, 0.0)
    assert [e for e in evs if e["type"] == "window"][0]["offline"] is True

    evs = _feed(f, 0.05, 0.05, 0.05, t=1)
    assert [e for e in evs if e["type"] == "window"][0]["offline"] is False

    f.update(ac, vp, sem, 2.0)
